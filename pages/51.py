...
# --- Module List ---
modules = sorted(
    [m for m in os.listdir(TRAINING_DIR) if os.path.isdir(os.path.join(TRAINING_DIR, m))],
    key=lambda x: x.lower()
)

# --- Initialize selected_module in session_state if needed ---
if modules and "selected_module" not in st.session_state:
...
with st.sidebar:
    ...
    if not st.session_state.questions:
        if st.button("📋 Start Quiz", key="start_quiz_btn"):
            ...
            with st.spinner("Analyzing training material and generating questions..."):
