...
def process_chart_tags(response_text, dataframe_dict):
    ...
    st.session_state.charts_rendered_by_tags = bool(st.session_state.charts_to_render)
...
if prompt:
    ...
    if st.session_state.project_dataframe is not None:
        ...
        for label in export_matches:
            ...
            # If filtering failed, fallback to full DataFrame
            final_df = filtered_df if filtered_df is not None and not filtered_df.empty else st.session_state.combined_df
            
            # Cache it for later reference
            st.session_state.export_results[label] = final_df

            # Replace tag with visual anchor
            response_text = response_text.replace(
                f"[EXPORT_CSV:{label}]",
                f"📥 Preview your download below **{label}** — see the preview and link below."
            )

            # Immediately render preview + download link
            #st.markdown(f"#### 📤 Export Preview: {label}")
            #st.dataframe(final_df.head(10))
            #export_dataframe_to_csv(final_df, filename=f"{label.replace(' ', '_')}.csv")
...
for i, message in enumerate(st.session_state.project_messages):
    ...
    if is_latest and message["role"] == "assistant":
        ...
        # ✅ Render export previews at the bottom
        if st.session_state.export_results:
            st.markdown("---")
            st.markdown("### 📤 Exported Results")

            for label, df in st.session_state.export_results.items():
                st.markdown(f"#### 🔽 Export Preview: {label}")
                st.dataframe(df.head(10))
                export_dataframe_to_csv(df, filename=f"{label.replace(' ', '_')}.csv")

            # ✅ Clear both after rendering
            st.session_state.charts_to_render = []
            st.session_state.export_results = {}

        # ✅ Detect auto-overview fallback charts
        is_auto_overview = (
        ...
        if (
            ...
            and not st.session_state.get("charts_rendered_by_tags", False)
        ):
